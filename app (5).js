...
let votesMap = {};          // { photoId: Map(voterId -> created_at ISO string) }
...
async function loadVotes() {
  const { data, error } = await sb.from("votes").select("photo_id, voter_id, created_at");
  if (error) { console.error(error); return; }
  votesMap = {};
  (data || []).forEach((v) => {
    if (!votesMap[v.photo_id]) votesMap[v.photo_id] = new Map();
    votesMap[v.photo_id].set(v.voter_id, v.created_at);
  });
}
...
// Comprime imagem para no máximo maxWidth px e qualidade JPEG reduzida
// Garante que fotos pesadas de celular sejam reduzidas antes do upload
async function compressImage(file, maxWidth = 1200, quality = 0.8) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // Reduz qualidade até atingir tamanho razoável
      let q = quality;
      let dataUrl = canvas.toDataURL("image/jpeg", q);
      while (dataUrl.length > 1_500_000 && q > 0.3) {
        q -= 0.1;
        dataUrl = canvas.toDataURL("image/jpeg", q);
      }
      // Converte dataUrl de volta pra Blob/File
    ...
    img.onerror = () => { URL.revokeObjectURL(url); resolve(file); }; // fallback: usa original
    img.src = url;
  });
}
...
async function toggleVote(photoId, userId) {
  ...
  if (already) {
    ...
    // 23505 = violação de chave única (já votou). Não é um erro real para o usuário,
    // só significa que um clique duplo/duplicado tentou inserir o mesmo voto de novo.
    if (error && error.code !== "23505") throw error;
  }
}

// Vota numa foto sem permitir desfazer (uso na tela de votação das alunas)
async function castVote(photoId, userId) {
  const { error } = await sb.from("votes").insert({ photo_id: photoId, voter_id: userId });
  if (error && error.code !== "23505") throw error;
}
...
function renderForgotPassword() {
  root.innerHTML = `
    <div class="container-sm" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;">
      ...
      <div class="card" style="width:100%;">
        <h3 style="color:var(--vinho);margin-top:0;margin-bottom:8px;">Recuperar senha</h3>
        <p class="sans" style="font-size:13px;color:var(--texto-suave);margin-bottom:18px;">
          Digite o e-mail da sua conta. Vamos te enviar um link para você criar uma nova senha.
        </p>
        ...
        <div id="reset-ok" class="sans hidden" style="font-size:13px;color:var(--verde);margin-bottom:14px;">
          ✅ E-mail enviado! Confira sua caixa de entrada (e o spam) e clique no link para criar uma nova senha.
        </div>
        <button id="reset-submit" style="width:100%;padding:12px;">Enviar link de recuperação</button>
        <p id="back-to-login" class="sans" style="font-size:13px;color:var(--vinho);text-align:center;margin-top:14px;cursor:pointer;text-decoration:underline;">
          Voltar para o login
        </p>
      </div>
    </div>`;
  ...
  document.getElementById("reset-submit").onclick = async () => {
    ...
    try {
      ...
      console.error(e);
      errEl.textContent = "Não foi possível enviar o e-mail. Tente novamente em instantes.";
      btn.disabled = false;
...
function renderResetPassword() {
  root.innerHTML = `
    <div class="container-sm" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;">
      ...
      <div class="card" style="width:100%;">
        <h3 style="color:var(--vinho);margin-top:0;margin-bottom:8px;">Criar nova senha</h3>
        <p class="sans" style="font-size:13px;color:var(--texto-suave);margin-bottom:18px;">
        ...
        <div id="newpass-err" class="err-msg"></div>
        <button id="newpass-submit" style="width:100%;padding:12px;">Salvar nova senha</button>
      </div>
    </div>`;

  document.getElementById("newpass-submit").onclick = async () => {
    const errEl = document.getElementById("newpass-err");
...
function renderLogin() {
  root.innerHTML = `
    <div class="container-sm" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;">
      ...
      <div class="card" style="width:100%;">
        ...
        <p id="forgot-link" class="sans" style="font-size:13px;color:var(--vinho);text-align:center;margin-top:14px;cursor:pointer;text-decoration:underline;">
          Esqueci minha senha
        </p>
  ...
  function renderFields() {
    ...
    document.getElementById("forgot-link").classList.toggle("hidden", mode !== "login");
  }
  renderFields();
  ...
  async function handleSave() {
    ...
    try {
      ...
      for (const sf of stagedFiles) {
        saveBtn.textContent = `Enviando foto ${stagedFiles.indexOf(sf) + 1} de ${stagedFiles.length}...`;
...
function renderVote(el) {
  ...
  function voteCreatedAt(photoId) {
    return votesMap[photoId] ? votesMap[photoId].get(currentUser.id) : null;
  }
  function canStillUndo(photoId) {
    const createdAt = voteCreatedAt(photoId);
  ...
  el.innerHTML = `
    <div class="container">
      ...
      ${!votingClosed ? `<p class="sans" style="font-size:12px;color:var(--texto-suave);margin-bottom:16px;">💡 Você pode desfazer um voto em até 10 minutos depois de votar. Depois disso, o voto fica definitivo.</p>` : ""}
      ...
      <div class="vote-grid">
        ${allPhotos
          .map((photo, idx) => {
            ...
            const undoable = voted && canStillUndo(photo.id);
            let btnHtml = "";
            if (!votingClosed) {
              if (!voted) {
                btnHtml = `<button class="vote-btn not-voted" data-photo="${photo.id}" data-action="vote">Votar</button>`;
              } else if (undoable) {
                btnHtml = `<button class="vote-btn voted" data-photo="${photo.id}" data-action="undo">✓ Votado (desfazer)</button>`;
              } else {
                btnHtml = `<button class="vote-btn voted" data-photo="${photo.id}" disabled>✓ Votado</button>`;
              }
            } else {
              btnHtml = `<div class="sans" style="text-align:center;font-size:13px;color:var(--texto-suave);font-weight:700;">${count} voto${count !== 1 ? "s" : ""}</div>`;
            }
            return `
  ...
  el.querySelectorAll(".vote-btn").forEach((btn) => {
    btn.onclick = async () => {
      ...
      try {
        if (action === "undo") {
          const { error } = await sb.from("votes").delete().eq("photo_id", photoId).eq("voter_id", currentUser.id);
          if (error) throw error;
          showToast("Voto desfeito.");
        } else {
          await castVote(photoId, currentUser.id);
        }
      } catch (e) {
        console.error(e);
        showToast("Não foi possível concluir a ação. Verifique sua conexão e tente novamente.", "err");
        btn.disabled = false;
      }
      voteInFlight = false;
    };
  ...
  if (!votingClosed) {
    const undoableTimes = allPhotos
      .filter((p) => hasVoted(p.id) && canStillUndo(p.id))
      .map((p) => new Date(voteCreatedAt(p.id)).getTime() + UNDO_WINDOW_MS - Date.now())
      .filter((ms) => ms > 0);
    if (undoableTimes.length > 0) {
      const nextExpiry = Math.min(...undoableTimes) + 250;
      clearTimeout(renderVote._timer);
      renderVote._timer = setTimeout(() => {
        if (currentView === "vote") renderVote(el);
      }, nextExpiry);
    }
  }
}
...
function renderResults(el) {
  ...
  const totalVoters = new Set(Object.values(votesMap).flatMap((m) => [...m.keys()])).size;
